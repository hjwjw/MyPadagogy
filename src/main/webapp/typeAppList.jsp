			...
			<c:forEach items="${appItemList}" var="appItem">
				<article>
					...
					<div class="art-content">
						...
						<!-- <script>
							var introduce = ${appItem.introduce };
							introduce = introduce.substr();
						</script> -->
			  ...
			  <center>
					<ul class="pagination">
						<li
								...
								<a href="type/queryByType/${type.typeId }?pageNum=${page.firstPage } " alt="首页">&laquo;</a>
						</li>
						<c:forEach items="${page.navigatepageNums }" var="i">
							<li <c:if test="${page.pageNum eq i}"> class="active" </c:if>>
									<a href="type/queryByType/${type.typeId}?pageNum=${i} ">${i} <span class="sr-only">(current)</span></a>
							</li>
						</c:forEach>
						<li
							<c:if test="${page.pageNum eq page.lastPage}"> class='disabled' </c:if>><a
							href="type/queryByType/${type.typeId}?pageNum=${page.lastPage} " alt="最后一页">&raquo;</a></li>
					</ul>
				</center>
			</div>
		</div>
